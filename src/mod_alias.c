...
static void alias_cmd(const char* chan, const char* name, const char* arg, int cmd){
    ...
    switch(cmd){
        ...
        case ALIAS_LIST: {
            ...
            //TODO(chronister): Need a solution for longer-than-1-message alias lists
            ...
            if(total == 0){
                ...
                for(int i = 0; i < total; ++i){
                    ...
                    for(int j = 0; j < subtotal; ++j){
                        ...
                        if ((int)sz - (int)strlen(alias_keys[i][j]) > 0) {
                            snprintf_chain(&ptr, &sz, "!%s%s", alias_keys[i][j], last ? "." : ", ");
                        }
                    }
#else
                    const bool last = (i == total - 1);
                    if ((int)sz - (int)strlen(alias_keys[i][0]) > 0) {
                        snprintf_chain(&ptr, &sz, "!%s%s", alias_keys[i][0], last ? "." : ", ");
                    }
#endif
                }
            }
